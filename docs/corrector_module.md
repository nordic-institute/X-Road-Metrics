...
This sections describes the necessary steps to install the **corrector module** on Ubuntu 22.04 (Jammy) or Ubuntu 24.04 (Noble). For a complete overview of different modules and machines, please refer to the ==> [System Architecture](system_architecture.md) <== documentation.
...
```bash
sudo apt update
sudo apt install xroad-metrics-corrector
```
