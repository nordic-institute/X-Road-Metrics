...
def mock_collector(mocker):
    mocker.patch('main.collector_worker.main')
    mocker.patch('main.update_servers.update_database_server_list')
    mocker.patch('main.update_servers.print_server_list')
...
def mock_settings_manager(mocker):
    ...
    mocker.patch('main.OpmonSettingsManager', return_value=mock_settings_manager)
    return mock_settings_manager


def test_action_collect(mocker, mock_collector, mock_settings_manager):
    mocker.patch('main.settings_action_handler')
    mocker.patch('sys.argv', ['test_program_name', '--profile', 'TEST', 'collect'])

    main.main()
    main.OpmonSettingsManager.assert_called_once_with('TEST')
    main.collector_worker.main.assert_called_once_with('testsettings')
    main.update_servers.update_database_server_list.assert_not_called()
    main.update_servers.print_server_list.assert_not_called()
...
def test_action_update(mocker, mock_collector, mock_settings_manager):
    ...
    main.main()
    main.OpmonSettingsManager.assert_called_once_with('TEST')
    main.collector_worker.main.assert_not_called()
    main.update_servers.update_database_server_list.assert_called_once_with('testsettings')
    main.update_servers.print_server_list.assert_not_called()
    main.settings_action_handler.assert_not_called()
...
def test_action_list(mocker, mock_collector, mock_settings_manager):
    ...
    main.collector_worker.main.assert_not_called()
    main.update_servers.update_database_server_list.assert_not_called()
    main.update_servers.print_server_list.assert_called_once_with('testsettings')
    main.settings_action_handler.assert_not_called()
