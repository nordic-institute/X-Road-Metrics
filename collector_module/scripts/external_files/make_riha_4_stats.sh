...
# Output file: riha_4_networking.json, hardcoded into ./subsystems_json_riha_4_networking.py
#
INPUT_RIHA="./riha_systems.json"
OUTPUT_RIHA="./riha_4_stats.txt"
...
# Do the job
python3 ./subsystems_json_riha_4_stats.py -s ${SECURITY_SERVER_URL} --riha ${INPUT_RIHA} # && \
# python3 ./subsystems_json_riha_4_networking.py -c ${CENTRAL_SERVER} --riha ${INPUT_RIHA} # && \
...
then
    # Specific for RIA
    # TMP="/tmp/$$"
    # cat ${OUTPUT_RIHA} | sed -e "s/\"email\": \[\]/\"email\": \[ \{\"name\": \"opmon-reports\", \"email\": \"toomas.molder@ria.ee\"\} \]/" > ${TMP} && mv ${TMP} ${OUTPUT_RIHA}

    # Check, printout
    echo "`date "+[%F %T]"` Info: Number of subsystems: `cat ${OUTPUT_RIHA} | egrep "subsystem_code" | wc -l`"
    # echo "`date "+[%F %T]"` Info: Number of subsystems without contact: `cat ${OUTPUT_RIHA} | egrep "\"email\": \[ {\"name\": \"opmon-reports\", \"email\": \"toomas.molder@ria.ee\"} \]" | wc -l`"
    ...
    echo "ERROR: ${SCRIPT} while make ${OUTPUT_RIHA}, ${TOTAL_TIME}"
fi
