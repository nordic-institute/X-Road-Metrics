...
"
        ...
        # SECURITY_SERVER_URL = \"http://10.0.15.141\"
        #
...
# Output file: riha_4_reports.json, hardcoded into ./subsystems_json_riha_4_reports.py
#
INPUT_RIHA="./riha_systems.json"
OUTPUT_RIHA="./riha_4_reports.json"
...
# Do the job
python3 ./subsystems_json_riha_4_reports.py -s ${SECURITY_SERVER_URL} --riha ${INPUT_RIHA} # && \
# python3 ./subsystems_json_riha_4_reports.py -c ${CENTRAL_SERVER} --riha ${INPUT_RIHA} # && \
...
then
    # Specific for RIA
    TMP="/tmp/$$"
    cat ${OUTPUT_RIHA} | sed -e "s/\"email\": \[\]/\"email\": \[ \{\"name\": \"RIA kasutajatugi\", \"email\": \"help@ria.ee\"\}, \{\"name\": \"opmon-reports\", \"email\": \"toomas.molder@ria.ee\"\} \]/" > ${TMP} && mv ${TMP} ${OUTPUT_RIHA}
