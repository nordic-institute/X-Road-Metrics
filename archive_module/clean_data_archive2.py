...
def process_archive(total_to_archive, minimum_to_archive, mdb_database, mdb_user, mdb_pwd, mdb_server, mdb_auth):
    ...
    try:
        with open(countfile, 'r') as f:
            ...
            f.close()
            print('{}\t- Total documents ready to archive {:n} (based from file {}, {} day(s) ago'.format(datetime.now().strftime("%Y-%m-%dT%H:%M:%S"), total_docs, countfile, days))
    except IOError:
        # print('No {} available at the moment'.format(countfile))
        # Read new value of total_docs from MongoDb and continue running
        print('{}\t- Connecting to database: {}, query db.clean_data.find({}).count(), {} day(s) ago'.format(datetime.now().strftime("%Y-%m-%dT%H:%M:%S"), db_name, db_str, days))
        total_docs = clean_data.find(db_str).count()
    ...
    if total_docs < minimum_to_archive:
        print('{}\t- The total of {:n} queries is smaller than the minimum of {:n}'.format(datetime.now().strftime("%Y-%m-%dT%H:%M:%S"), total_docs, minimum_to_archive))
        # delete countfile
        try:
            os.remove(countfile)
