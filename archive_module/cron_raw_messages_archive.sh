...
cd ${DIR}
# Name of subdirectory (under RAW_MESSAGES_ARCHIVE_DIR/) where to move archive file
...
# We do not need CLEAN_DATA_ARCHIVE_DIR within this script
...
# Actual stuff
python3 -u ./raw_messages_archive.py \
    ${MONGODB_QUERY_DB} \
    ${MONGODB_USER} \
...
# Result of script should be available at ${RAW_MESSAGES_ARCHIVE_DIR}
# Move archive to ${SUBDIR}
echo -e "$(date +'%FT%T')\t${PID}: ${0} Move archive to ${RAW_MESSAGES_ARCHIVE_DIR}/${SUBDIR}/"  | tee -a ${LOG}
for file in $(find ${RAW_MESSAGES_ARCHIVE_DIR}/raw_archive_query_db_${INSTANCE}_*.json.gz -newer ${LOCK}); do echo "=== ${file} ==="; SUBDIR=$(date -d "@$(stat -c '%Y' ${file})" '+%Y/%m/%d'); mkdir -p ${RAW_MESSAGES_ARCHIVE_DIR}/${SUBDIR}; mv ${file} ${RAW_MESSAGES_ARCHIVE_DIR}/${SUBDIR}; done 2>&1 | tee -a ${LOG}

# Remove ${LOCK}
/bin/rm ${LOCK} 2>&1 | tee -a ${LOG}

# The End
# df ${RAW_MESSAGES_ARCHIVE_DIR} | tee -a ${LOG}
